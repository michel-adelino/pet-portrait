...
export interface TShirtMockupRef {
  ...
  getFullMockupCanvas: () => HTMLCanvasElement | null;
...
const TShirtMockup = forwardRef<TShirtMockupRef, TShirtMockupProps>(({
  ...
  const getFullMockupCanvasRef = useRef<() => HTMLCanvasElement | null>(null);
  ...
  const isExporting = exporting || isCapturingForExport;
  ...
  // When capturing for download, re-render then capture (so rectangle is hidden)
  useEffect(() => {
    if (!isCapturingForExport) return;
    const timer = requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        const canvas = getFullMockupCanvasRef.current?.() ?? canvasRef.current?.getStageCanvas();
        if (canvas) {
  ...
  useImperativeHandle(ref, () => ({
    ...
    getFullMockupCanvas: (() => {
      const fn = () => {
        const stageCanvas = canvasRef.current?.getFullStageCanvas();
        const img = shirtImgRef.current;
        if (!stageCanvas) return null;
        const pixelRatio = 2;
        const w = containerWidth * pixelRatio;
        const h = 500 * pixelRatio;
      ...
      getFullMockupCanvasRef.current = fn;
      return fn;
    })(),
